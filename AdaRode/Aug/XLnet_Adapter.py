...
class RodeXLAdapter:
    ...
    def get_pred(self, input):
        ...
        with torch.no_grad():
            outputs = self.model(res["input_ids"], attention_mask=res["attention_mask"])
    ...
    def get_prob(self, input):
        ...
        return prob.cpu().numpy().flatten().tolist()

    # 新增批量预测类别
    def get_pred_batch(self, inputs):
        res = self.tokenizer(inputs, padding=True, truncation=True, max_length=128, return_tensors="pt").to(device)
        with torch.no_grad():
            outputs = self.model(res["input_ids"], attention_mask=res["attention_mask"])
    ...
    # 新增批量预测概率
    def get_prob_batch(self, inputs):
        res = self.tokenizer(inputs, padding=True, truncation=True, max_length=128, return_tensors="pt").to(device)
        with torch.no_grad():
            outputs = self.model(res["input_ids"], attention_mask=res["attention_mask"])
        prob = F.softmax(outputs, dim=1)
