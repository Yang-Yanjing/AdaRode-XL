...
class MHM:
    ...
    def result_caculate(self):
        ...
        # 保存 SQL 攻击统计信息
        with open(f"Result/sql_attack_summary_{timestamp}.json", "w", encoding="utf-8") as f_sql:
            json.dump(result["sql"]["AF"], f_sql, ensure_ascii=False, indent=4)

        # 保存 XSS 攻击统计信息
        with open(f"Result/xss_attack_summary_{timestamp}.json", "w", encoding="utf-8") as f_xss:
    ...
    def _replaceWords(self, _vic, _viclabel, datatype="xss", _prob_threshold=0.95, _candi_mode="allin", _victim=None):
        ...
        if self._filter_attack_order(_vic) == "selected":
            ...
            if _candi_mode == "allin":
                ...
                # 批量获取概率与预测
                probs = np.array(victim.get_prob_batch(_candi_tokens))  # shape: (len(_candi_tokens), num_classes)
                preds = victim.get_pred_batch(_candi_tokens)            # shape: (len(_candi_tokens),)
...
if __name__ == "__main__":
    for i in range(1,11):
        ...
        test_data = pd.read_csv("Data/Test/test_set_split"+str(i)+".csv")
